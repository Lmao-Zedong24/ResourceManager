...
const float PlayerGO::CAM_SPEED			= 20;
const std::string PlayerGO::CameraName	= "PlayerCamera";

GLFWwindow* PlayerGO::window = nullptr;
...
void PlayerGO::Inputs(GLFWwindow* p_window)
{
	...
	glfwSetCursorPos(p_window, (1920 / 2), (1080 / 2));
}
...
void PlayerGO::Update()
{
	...
	//{
	//	auto& rm = Multi::OpenGLGM::GetInstance()->GetRM();
	//	auto cam = this->GetChild<CameraG0>(CameraName);
	//	auto cursor1 = cam->GetChild<CameraG0>("crosshair1");
	//	auto cursor2 = cam->GetChild<CameraG0>("crosshair2");
